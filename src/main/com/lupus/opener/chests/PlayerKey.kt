 ...
 */
class PlayerKey : ConfigurationSerializable {
    var player: UUID?
    var amount: Int

    constructor(player: Player, amountKeys: Int) : this(player.uniqueId, amountKeys) {}
    constructor(player: UUID?, amountKeys: Int) {
        this.player = player
        amount = amountKeys
    }

    constructor(map: Map<String?, Any?>) {
        player = UUID.fromString(map["UUID"] as String?)
        amount = map["amount"] as Int
    }

    override fun serialize(): Map<String, Any> {
        val map: MutableMap<String, Any> = HashMap()
        map["UUID"] = player.toString()
        map["amount"] = amount
        return map
    }
}