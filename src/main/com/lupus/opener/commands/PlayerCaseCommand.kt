...
class PlayerCaseCommand : PlayerSupCommand(meta, arrayOf<LupusCommand>(
    BuyKeyCMD(),
    ChangeKeyCMD(),
    KeyTopCMD(),
    KeyTransactionCMD(),
    WithdrawKeyCommand())) {
    override fun optionalOperations(sender: CommandSender, args: ArgumentList): Boolean {
        if (sender is Player) {
            if (args.size < 1) {
                val player = sender
                val list = ChestList(Message.CHEST_LIST_INVENTORY_NAME.toString(), player)
                list.open(player)
                return true
            }
        }
    ...
    companion object {
        var meta = CommandMeta().setName("skrzynie").setUsage(usage("/skrzynie"))
            .setDescription("Główna komenda dla skrzyń dla gracza").setArgumentAmount(0)
    }
}