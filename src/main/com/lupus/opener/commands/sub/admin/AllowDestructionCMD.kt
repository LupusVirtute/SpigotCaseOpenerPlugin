...
class AllowDestructionCMD : PlayerCommand(meta) {
    @Throws(Exception::class)
    override fun run(executor: Player, args: ArgumentList) {
        val argument = args.getArg(String::class.java, 0)
        if (argument.contains("yes")) {
            BlockManipulationListener.Companion.isTimeForDestroy = true
            executor.sendMessage(colorText(Message.DESTROY_MESSAGE_ON.toString()))
        } else {
            BlockManipulationListener.Companion.isTimeForDestroy = false
            executor.sendMessage(colorText(Message.DESTROY_MESSAGE_OFF.toString()))
        }
    }

    companion object {
        var meta = CommandMeta().setName("destroy").setUsage(usage("/case destroy", "[yes/no]")).setDescription(
            colorText("&6Pozwala lub nie pozwala na niszczenie miejsc skrzyń &a&lyes&0&l/&4&lno"))
            .addPermission("case.admin.remove").setArgumentAmount(1)
    }
}