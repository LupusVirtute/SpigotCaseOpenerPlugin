...
class EditWeightCMD : LupusCommand(meta) {
    @Throws(Exception::class)
    override fun run(commandSender: CommandSender, args: ArgumentList) {
        val chest = args.getArg(String::class.java, 0)
        val weight = args.getArg(Int::class.javaPrimitiveType, 1)
        val minecraftCase = ChestManager.getCase(chest)
        if (minecraftCase == null) {
            val mrq = MessageReplaceQuery().addQuery("chest", chest)
    ...
    companion object {
        var meta = CommandMeta().addPermission("case.admin").setName("editweight")
            .setDescription(colorText("&4&lEdytuje wage skrzyni")).setUsage(
            usage("/case editweight", "[case] [newweight]")).setArgumentAmount(2)
    }
}