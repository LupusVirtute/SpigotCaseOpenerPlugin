...
class GetCaseCMD : PlayerCommand(meta) {
    @Throws(Exception::class)
    public override fun run(executor: Player, args: ArgumentList) {
        val chestName = args.getArg(String::class.java, 0)
        val stackAmount = args.getArg(Int::class.javaPrimitiveType, 1)
        val exists = ChestManager.contains(chestName)
        if (!exists) {
    ...
    companion object {
        var meta = CommandMeta().setName("get").setUsage(usage("/case get", "[nazwa] [ilosc]")).setDescription(
            colorText("&9Dostajesz skrzynie z ktorej bedzie mozna otwierac")).addPermission("case.admin.get")
            .setArgumentAmount(2)
    }
}