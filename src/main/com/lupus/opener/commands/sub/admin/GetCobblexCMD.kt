...
class GetCobblexCMD : PlayerCommand(meta) {
    @Throws(Exception::class)
    override fun run(player: Player, argumentList: ArgumentList) {
        val mcCase = argumentList.getArg(MinecraftCase::class.java, 0) ?: return
        val item = mcCase.cobblex
        InventoryUtility.addItemStackToPlayerInventory(player, item)
    }

    companion object {
        var meta = CommandMeta().setName("cobblex").setUsage(usage("/case cobblex", "[case]")).setDescription(
            colorText("&6Dostajesz cobblex'a")).addPermission("case.admin.key.add").setArgumentAmount(0)
    }
}