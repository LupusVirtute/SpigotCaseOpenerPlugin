...
class OpenCaseCMD : PlayerCommand(meta) {
    ...
    companion object {
        var meta =
            CommandMeta().addPermission("case.admin.open").setName("open").setUsage(usage("/case open", "[name]"))
                .setDescription(
                    colorText("&6Otwierasz skrzynie test&5O&bw&5O")).setArgumentAmount(1)
    }
}