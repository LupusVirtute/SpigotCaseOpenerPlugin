...
class OpenEditorCMD : PlayerCommand(meta) {
    public override fun run(executor: Player, args: ArgumentList) {
        val list = ChestList(Message.CHEST_LIST_INVENTORY_NAME.toString(), executor)
        list.open(executor)
    }

    companion object {
        var meta = CommandMeta().setName("editor").setUsage(usage("/case editor"))
            .setDescription(colorText("&9Otwiera edytor skrzyń")).setArgumentAmount(0)
    }
}