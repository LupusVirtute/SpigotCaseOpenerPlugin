...
class ReloadAllCMD : PlayerCommand(meta) {
    public override fun run(executor: Player, args: ArgumentList) {
        CaseOpener.Companion.loadChests()
        Message.Companion.load()
    }

    companion object {
        var meta = CommandMeta().setName("reloadall").setUsage(usage("/case reloadall"))
            .setDescription(colorText("&6 Zapisuje samo tlumaczace sie")).addPermission("case.admin.save")
            .setArgumentAmount(0)
    }
}