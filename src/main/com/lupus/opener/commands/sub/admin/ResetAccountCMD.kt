...
class ResetAccountCMD : LupusCommand(meta) {
    @Throws(Exception::class)
    override fun run(commandSender: CommandSender, argumentList: ArgumentList) {
        val player = argumentList.getArg(OfflinePlayer::class.java, 0)
            ?: throw Exception("Player offline")
        for (mcCase in ChestManager.allCases) {
            mcCase.setKey(player.uniqueId, 0)
        }
        val mrq = MessageReplaceQuery().addQuery("player", player.name ?: "")
        commandSender.sendMessage(Message.COMMAND_KEYS_RESET.toString(mrq))
        mrq.addQuery("player", commandSender.name)
        if (player.isOnline) player.player?.sendMessage(Message.COMMAND_KEYS_RESET_PLAYER.toString(mrq))
    }

    companion object {
        var meta = CommandMeta().addPermission("case.admin.key.reset").setName("reset")
            .setUsage(usage("/case reset", "[player]")).setDescription(
            colorText("&6Resetujesz konto graczowi")).setArgumentAmount(1)
    }
}