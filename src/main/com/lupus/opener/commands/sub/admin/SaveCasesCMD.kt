...
class SaveCasesCMD : PlayerCommand(meta) {
    public override fun run(executor: Player, args: ArgumentList) {
        ChestManager.saveAll(true)
    }

    companion object {
        var meta = CommandMeta().setName("saveall").setUsage(usage("/case saveall"))
            .setDescription(colorText("&6 Zapisuje samo tlumaczace sie")).setArgumentAmount(0)
