...
class SetIconCMD : PlayerCommand(meta) {
    @Throws(Exception::class)
    override fun run(player: Player, argumentList: ArgumentList) {
        val mcCase = argumentList.getArg(MinecraftCase::class.java, 0)
    ...
    companion object {
        var meta = CommandMeta().setName("seticon").setUsage("/case seticon [case]").setArgumentAmount(1)
    }
}