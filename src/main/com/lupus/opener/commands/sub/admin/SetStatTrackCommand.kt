...
class SetStatTrackCommand : PlayerCommand(meta) {
    ...
    override fun run(player: Player, argumentList: ArgumentList) {
        ...
        item = CaseItem.Companion.addStarTrack(item)
        inventory.setItemInMainHand(item)
        player.sendMessage(Message.STATTRACK_ITEM_SET_PROPERLY.toString())
    }

    companion object {
        var meta = CommandMeta().setName("stattrack").setDescription("Sets stat track on item held in hand").setUsage(
            usage("/case stattrack")).setArgumentAmount(0)
    }
}