...
class ChangeKeyCMD : PlayerCommand(meta) {
    @Throws(Exception::class)
    override fun run(player: Player, args: ArgumentList) {
        val chestFromName = args.getArg(String::class.java, 0)
        val chestToName = args.getArg(String::class.java, 1)
        val amount = args.getArg(Int::class.javaPrimitiveType, 2)
        val mcCase = ChestManager.getCase(chestFromName)
        if (mcCase == null) {
            val mrq = MessageReplaceQuery().addQuery("chest", chestFromName)
            player.sendMessage(Message.CASE_GIVEN_DONT_EXISTS.toString(mrq))
            return
        }
        val forCase = ChestManager.getCase(chestToName)
        if (forCase == null) {
            val mrq = MessageReplaceQuery().addQuery("chest", chestToName)
            player.sendMessage(Message.CASE_GIVEN_DONT_EXISTS.toString(mrq))
            return
        }
        if (forCase.caseWeight < mcCase.caseWeight) return
        val weightOfCase = mcCase.caseWeight / 16
        val neededAmount = forCase.caseWeight / weightOfCase * amount
        if (neededAmount > mcCase.getKeyAmount(player)) {
            val mrq = MessageReplaceQuery().addQuery("amount", neededAmount.toString() + "")
        ...
        val mrq = MessageReplaceQuery().addQuery("needed_amount", neededAmount.toString() + "")
            .addQuery("chest", chestFromName).addQuery("amount", amount.toString() + "").addQuery("chest2", chestToName)
        player.sendMessage(Message.COMMAND_CHANGE_KEY_SUCCESS.toString(mrq))
    }

    companion object {
        var meta = CommandMeta().setName("zamienklucz")
            .setUsage(usage("/skrzynie zamienklucz", "[zskrzyni] [naskrzynie] [ilosc]")).setDescription(
            colorText("&6&lNarazie brak spisu ile na jaka skrzynie trzeba"))
            .setArgumentAmount(3)
    }
}