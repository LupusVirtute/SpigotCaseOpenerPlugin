...
class BuyKeysCMD : GUI("Klucze", 36) {
    ...
    init {
        for (allCase in ChestManager.allCases) {
            val item = ItemStack(Material.CHEST)
            val mrq = MessageReplaceQuery().addQuery("price", df2.format(allCase.price))
            ...
            ItemUtility.setItemTitleAndLore(
                item,
                allCase!!.officialName1,
                lore
            )
            addItemStack(SelectableCommand(item, "kupklucz " + allCase.name + " 1"))
        }
    }
}