...
class TopKeysGUI(caller: Player?) : Paginator(Message.TOP_KEYS_MAIN_INVENTORY_NAME.toString()) {
    override fun onClose(player: Player) {}

    companion object {
        var selectableTops: MutableList<SelectableTop> = ArrayList()
    }

    init {
        for (item in selectableTops) {
            addItemStack(item)
        }
        setPage(0)
    }
