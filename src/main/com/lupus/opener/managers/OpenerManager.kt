...
object OpenerManager {
    private val playerOpener = HashMap<UUID, OpeningCase?>()
    fun getPlayerOpeningCase(p: UUID): OpeningCase? {
        return playerOpener[p]
    }

    fun getPlayerOpeningCase(p: Player): OpeningCase? {
        return getPlayerOpeningCase(p.uniqueId)
    }

    fun setPlayerOpener(p: Player, mcCase: OpeningCase?) {
        setPlayerOpener(p.uniqueId, mcCase)
    }

    fun removePlayerOpener(p: Player) {
        playerOpener.remove(p.uniqueId)
    }

    fun setPlayerOpener(p: UUID, mcCase: OpeningCase?) {
        playerOpener[p] = mcCase
    }
}