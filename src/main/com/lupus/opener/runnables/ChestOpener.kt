...
class ChestOpener(var mcCase: OpeningCase, var forPlayer: Player) : BukkitRunnable() {
    override fun run() {
        if (!forPlayer.isOnline) {
            return
        }
        val b = mcCase.doABarrelRoll(forPlayer)
        val time = (mcCase.currentIndex as Float / mcCase.winnerIndex as Float * 9f).toInt() + 1
        if (!b) ChestOpener(mcCase, forPlayer).runTaskLater(CaseOpener.mainPlugin, time.toLong())
    }
}