...
class ChestSave(var mcCases: HashMap<String, MinecraftCase>) : BukkitRunnable() {
    override fun run() {
        var time = System.currentTimeMillis()
        Bukkit.broadcastMessage(Message.SAVING_INIT.toString())
        for (actualCase in mcCases.values) {
            actualCase.save()
        }
        time = (System.currentTimeMillis() - time) / 1000
        val mrq = MessageReplaceQuery().addQuery("amount", time.toString() + "")
